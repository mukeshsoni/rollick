...
- [ ] Render components in iframe for isolation of css/javascript
  ...
  - [ ] See how we can inject the css required in a component into the iframe
    - [ ] Rithwik has another solution. What if the fetching of components is done inside the preview component. 
...
https://github.com/systemjs/plugin-css/blob/master/README.md
    - [ ] Use serviceworker to intercept less/css calls and supply the raw contents of the file
      - [ ] css
      - [ ] less file
        - [ ] How to handle further resolution of less dependencies? Should less be transpiled in the frontend?
