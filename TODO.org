...
- [ ] Rollick install should not fail if it doesn't find any config file.
It should still give the bare minimum experience, which is similar to codepen. 

And then the tool should provide options to build up the config through various ui options.
E.g. option to add a component source path which will generate the component meta data in
the backend and save it (either in local storage or a db)

That will intact make it possible to add more than one list of components from different parts of the file system.
In fact, rollick can then be installed centrally and can have it's own db on the os,
but still allow configuration to allow using a different db.
